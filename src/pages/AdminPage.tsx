...
import { useNavigate } from 'react-router-dom';
import { 
  Home, BarChart3, FileText, Mail, LogOut, Users, TrendingUp, 
  AlertTriangle, RefreshCw, Loader2, Filter, CheckCircle, Info, 
  Zap, AlertCircle, Target, Globe, Lightbulb, Award, Database, 
  CheckSquare, MessageSquare, Search, Smile, Repeat, Heart, 
  Compass, Bell, Calendar, Percent, Upload, Download, FileSpreadsheet 
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
...
import { EmployeeMapping } from '@/services/mappingApi';
import { useAuth } from '@/hooks/useAuth';
import BulkUploadAnalytics from '@/components/BulkUploadAnalytics';
import {
  BarChart,
  Bar,
  XAxis,
...
type AdminTab = 'home' | 'analytics' | 'logs' | 'reminders' | 'sheets' | 'bulk-upload';
      ...
      if (result.success && result.data) {
        const enrichedResponses = result.data.map(response => {
          ...
          if (matchingEmployee && matchingEmployee.Client) {
            ...
            responseWithClient['Client'] = 'Unknown';
          }
      ...
      <aside className="w-64 bg-primary text-white flex flex-col">
        ...
        <nav className="flex-1 py-4">
          {[
            { id: 'home', icon: Home, label: 'Dashboard' },
            { id: 'analytics', icon: BarChart3, label: 'Analytics' },
            { id: 'bulk-upload', icon: Upload, label: 'Bulk Upload Analytics' },
            { id: 'logs', icon: FileText, label: 'Logs' },
            { id: 'reminders', icon: Mail, label: 'Reminders' },
          ].map(item => (
            <button
      ...
      <main className="flex-1 p-8 bg-muted overflow-auto">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as AdminTab)} className="space-y-6">
          ...
          <TabsList>
            <TabsTrigger value="home">Dashboard</TabsTrigger>
            <TabsTrigger value="analytics">Analytics</TabsTrigger>
            <TabsTrigger value="bulk-upload">Bulk Upload Analytics</TabsTrigger>
            <TabsTrigger value="logs">Activity Logs</TabsTrigger>
            <TabsTrigger value="reminders">Reminders</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="home" className="space-y-6">
            <div className="animate-fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                ...
                {/* Card 2: Positive Feedback - Compact */}
                <Card className="hover:shadow-md transition-shadow">
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between">
                      <div>
                      ...
                      <div>
                        ...
                        <span className="text-2xl font-bold block leading-tight text-rose-600">
                          {(() => {
                            ...
                            const totalPossible = filteredResponses.length * 2;
                            return `${Math.round((criticalCount / Math.max(totalPossible, 1)) * 100)}%`;
                          })()}
                        </span>
                        <div className="mt-2 text-xs text-muted-foreground">
                ...
                <CardContent>
                  {(() => {
                    const calculateProcessParticipationFromResponses = () => {
                      ...
                      const processResponseCounts = new Map<string, number>();
                      const processUniqueSubmitters = new Map<string, Set<string>>();
                      const processSubmitterDetails = new Map<string, Map<string, number>>();
